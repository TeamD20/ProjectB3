...
public:
	...
	// CachedTargets를 TotalScore 기준 내림차순 상위 K개 반환
	TArray<FPBTargetScore> GetTopKTargets(int32 K = 3);

	/*~ DFS 다중 행동 탐색 인터페이스 ~*/

	// DFS + Branch & Bound 재귀 탐색.
	// Context는 값 복사로 백트래킹 자동화.
	...
	// 전술적 카이트 거리 기반 후퇴 위치 계산 (BestAttackRange × 0.85 클램프)
	UFUNCTION(BlueprintCallable, Category = "AI|Clearinghouse")
	...
	// EQS IdealDistance 테스트에서 참조할 이상적 교전 거리 (0 이하 = 선호도 없음)
	...
	// 공격 위치 EQS 쿼리 비동기 실행
	// MaxRange ≤ 0: 사거리 무제한, IdealDistance ≤ 0: 선호도 없음
...
protected:
	...
	// MultiTarget 어빌리티당 DFS 등록 상위 후보 수
